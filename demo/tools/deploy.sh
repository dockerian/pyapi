...
#   $1 - target feature identifier, e.g. myLambda (see deploy-foobar.json)
...
#   S3_BUCKET (optional, default to "dockerian")
...
function check_response() {
  ...
  for v in ${def_props} ${env_props} ${vpc_props}; do
    ...
    if [[ "${new_value}" != "${old_value}" ]]; then
      local changes="has changed to '${new_value}' - it was '${old_value}'"
      log_error "The config '.$v' ${changes}"
    fi
  done
...
function deploy_config() {
  ...
  local aws_cli="aws lambda update-function-configuration"
...
function get_command_args() {
  ...
  local DESCRIPTION="${ALF_DESC}_${ARG_ALIAS}_${ARG_BUILD}"
